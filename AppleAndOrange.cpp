...
int fruit_count(int s, int t, int a, vector<int> fruits){
    int hit_point,count = 0;
    for (int i:fruits){
       hit_point = i+a;  //final location of the fruit.
       //testing for the range.
       if(s <= hit_point && hit_point <= t){
           count++;
       }
       hit_point = 0; //resetting hit point for next iteration.
    }
    return count;
}
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    cout << fruit_count(s, t, a, apples)<<endl;   //checking apple count.
    cout << fruit_count(s, t, b, oranges)<<endl;  //checking oranges count. 
}
